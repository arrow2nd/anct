...
func PrintWorksTable(w io.Writer, works []*gen.WorkFragment) {
	if len(works) == 0 {
...
func PrintCharactersTable(w io.Writer, charachers []*gen.CharacterFragment) {
	if len(charachers) == 0 {
		fmt.Fprintln(w, "No matches found for your search")
		return
	}
	...
	printTable(w, []string{"NAME", "SERIES", "WORK ID"}, data)
}
